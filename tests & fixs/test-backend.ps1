...
Write-Host "`n[2] Testing GET /api/test/user/user1" -ForegroundColor Yellow
try {
    $response = Invoke-RestMethod -Uri "http://localhost:8080/api/test/user/user1" -Method GET
    Write-Host "Success! User found:" -ForegroundColor Green
    Write-Host "  Username: $($response.tenDn)" -ForegroundColor White
    Write-Host "  Password hash: $($response.mk)" -ForegroundColor White
} catch {
    Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Red
}

# Test 3: Test login endpoint
Write-Host "`n[3] Testing POST /api/auth/login" -ForegroundColor Yellow
try {
    $body = @{
        username = "user1"
        password = "admin123"
